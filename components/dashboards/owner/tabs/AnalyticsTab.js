...
export default function AnalyticsTab() {
    ...
    viewingRequests.forEach(request => {
      if (request.created_at) {
        ...
        if (!monthlyData[monthYear]) {
          monthlyData[monthYear] = { month: monthYear, viewings: 0, applications: 0 };
        }
  ...
  const occupiedProperties = properties.filter(p => p.status?.toLowerCase() === 'occupied').length;
  const vacantProperties = properties.filter(p => p.status?.toLowerCase() === 'available').length;
    ...
    <div className="bg-white shadow rounded-lg p-6">
      ...
      <div className="flex flex-wrap mb-6 gap-2">
        <button 
          ...
          className={`px-4 py-2 rounded-md ${activeView === "overview" 
        ...
        <button 
          ...
          className={`px-4 py-2 rounded-md ${activeView === "properties" 
            ? "bg-custom-red text-white" 
            : "bg-gray-100 hover:bg-gray-200 text-gray-700"}`}
        >
          Properties
        ...
        <button 
          ...
          className={`px-4 py-2 rounded-md ${activeView === "engagement" 
            ? "bg-custom-red text-white" 
            : "bg-gray-100 hover:bg-gray-200 text-gray-700"}`}
        >
          Engagement
        ...
        <button 
          ...
          className={`px-4 py-2 rounded-md ${activeView === "trends" 
            ? "bg-custom-red text-white" 
            : "bg-gray-100 hover:bg-gray-200 text-gray-700"}`}
        >
          Trends
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <div className="bg-blue-50 rounded-lg p-4 shadow-sm">
              <h3 className="text-lg font-medium text-blue-800">Properties</h3>
              <div className="mt-2 flex justify-between items-end">
                <div className="text-3xl font-bold text-blue-700">{totalProperties}</div>
                <div className="text-sm">
                  <div className="text-green-600">{vacantProperties} Available</div>
                  <div className="text-purple-600">{occupiedProperties} Occupied</div>
                </div>
              </div>
            </div>
            
            <div className="bg-green-50 rounded-lg p-4 shadow-sm">
              <h3 className="text-lg font-medium text-green-800">Viewing Requests</h3>
              <div className="mt-2 flex justify-between items-end">
                <div className="text-3xl font-bold text-green-700">{totalViewingRequests}</div>
                <div className="text-sm text-green-600">
                  {viewingRequests.filter(r => r.status?.toLowerCase() === 'pending').length} Pending
                </div>
              </div>
            </div>
            ...
            <div className="bg-purple-50 rounded-lg p-4 shadow-sm">
              ...
              <div className="mt-2 flex justify-between items-end">
                <div className="text-3xl font-bold text-purple-700">{rentalApplications.length}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
              <h3 className="text-lg font-medium text-gray-800 mb-4">Property Types</h3>
              <div className="h-64">
            ...
            {/* Property Engagement */}
            <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
              <h3 className="text-lg font-medium text-gray-800 mb-4">Request Status</h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
      ...
      {activeView === "properties" && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            {/* Property Pricing */}
            <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
              <h3 className="text-lg font-medium text-gray-800 mb-4">Property Pricing</h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
            ...
            {/* Bedrooms & Bathrooms */}
            <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
              <h3 className="text-lg font-medium text-gray-800 mb-4">Bedrooms & Bathrooms</h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={prepareBedBathData()}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="bedrooms" fill="#8884d8" name="Bedrooms" />
                    <Bar dataKey="bathrooms" fill="#82ca9d" name="Bathrooms" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg shadow-sm mb-6">
            <h3 className="text-lg font-medium text-gray-800 mb-4">Property Comparison</h3>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Size (sqft)</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {properties.slice(0, 5).map((property, index) => (
                    <tr key={property.id || index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                          ${property.status?.toLowerCase() === 'available' ? 'bg-green-100 text-green-800' : 
                            property.status?.toLowerCase() === 'occupied' ? 'bg-blue-100 text-blue-800' : 
      ...
      {activeView === "engagement" && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            {/* Property Engagement */}
            <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
              <h3 className="text-lg font-medium text-gray-800 mb-4">Property Engagement</h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={preparePropertyEngagementData()}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="viewings" name="Viewing Requests" fill="#8884d8" />
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {rentalApplications.slice(0, 5).map((app, index) => {
                    ...
                    return (
                      <tr key={app.id || index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
      ...
      {activeView === "trends" && (
        <>
          {/* Monthly Trends */}
          <div className="bg-gray-50 p-4 rounded-lg shadow-sm mb-6">
            <h3 className="text-lg font-medium text-gray-800 mb-4">Monthly Activity Trends</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={prepareTrendsData()}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="viewings" stroke="#8884d8" name="Viewing Requests" />
          ...
          <div className="bg-gray-50 p-4 rounded-lg shadow-sm mb-6">
            <h3 className="text-lg font-medium text-gray-800 mb-4">Property Performance Overview</h3>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Property</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Price</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Viewing Requests</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {properties.slice(0, 5).map((property, index) => {
                    ...
                    const conversionRate = propertyViewings > 0 
                      ? ((propertyApplications / propertyViewings) * 100).toFixed(1) 
                      : '0';
          ...
          {/* Market Insights */}
          <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
            <h3 className="text-lg font-medium text-gray-800 mb-4">Market Insights</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white p-4 rounded shadow-sm">
                <h4 className="font-medium text-gray-700 mb-2">Average Days to Rent</h4>
                ...
                <p className="text-sm text-gray-500 mt-1">Days from listing to occupation</p>
              </div>
              
              <div className="bg-white p-4 rounded shadow-sm">
                <h4 className="font-medium text-gray-700 mb-2">Average Rent Price</h4>
                <div className="text-2xl font-bold text-indigo-600">
              ...
              <div className="bg-white p-4 rounded shadow-sm">
                <h4 className="font-medium text-gray-700 mb-2">Occupancy Rate</h4>
                <div className="text-2xl font-bold text-indigo-600">
                  {totalProperties ? 
                    Math.round((occupiedProperties / totalProperties) * 100) : 0
                  }%
                </div>
                <p className="text-sm text-gray-500 mt-1">Percentage of occupied properties</p>
              </div>
            </div>
          </div>
        </>
      ...
      {properties.length === 0 && (
        <div className="bg-gray-50 p-6 rounded-lg text-center mt-6">
          ...
          <button 
            ...
            onClick={() => window.location.href = '/dashboard/owner?tab=properties'}
          >
