...
const DashboardSidebar = ({ activeTab, setActiveTab }) => {
  ...
  // Custom icon component that handles both image and fallback
  const IconComponent = ({ imageSrc, fallbackIcon, alt, className = "h-5 w-5 mr-3" }) => {
  ...
  // Define all navigation items with image icons
  const navItems = [
    // Admin tabs
    {
      id: "users",
    ...
    <>
      ...
      >
        ...
        <div className="p-6 flex flex-col items-center border-b border-custom-orange">
          ...
          {/* Role Badge */}
          <div
            className={`px-2 py-1 rounded-full text-xs font-medium mb-2 ${roleInfo.badgeColor}`}
          >
            {roleInfo.title}
          </div>
      ...
      {/* Profile Edit Modal */}
      <ProfileEditModal isOpen={showEditModal} onClose={handleModalClose} />
    </>
  );
};
