...
export const propertySearchService = {
  ...
  processFormOptionsFromRawData(rawData) {
    ...
    // Process locations - get unique, non-null locations
    ...
    // Process prices - calculate smart price ranges
    ...
    if (validPrices.length > 0) {
      ...
      const minPricePoints = this.generatePricePoints(minPrice, maxPrice * 0.8, 6);
      
      // Generate max prices (starting from ~20% above min, going to maximum)
      const maxPricePoints = this.generatePricePoints(minPrice * 1.2, maxPrice, 6);
    ...
    const uniqueBedrooms = bedroomsData
    ...
    const uniqueBathrooms = bathroomsData
    ...
    return {
      ...
      priceRanges: priceRanges.minPrices.length > 0 ? priceRanges : this.getFallbackPriceRanges(),
      bedroomOptions: uniqueBedrooms.length > 0 ? uniqueBedrooms : this.getFallbackBedrooms(),
      bathroomOptions: uniqueBathrooms.length > 0 ? uniqueBathrooms : this.getFallbackBathrooms(),
      ...
      stats: {
        ...
        bedroomRange: uniqueBedrooms.length > 0 ? {
          min: uniqueBedrooms[0],
          max: uniqueBedrooms[uniqueBedrooms.length - 1]
        } : null,
        bathroomRange: uniqueBathrooms.length > 0 ? {
  ...
  buildSearchFilters(formData) {
    ...
    if (formData.location && formData.location.trim() !== '') {
      filters.push({ column: 'location', operator: 'eq', value: formData.location });
    }
    
    if (formData.property_type && formData.property_type.trim() !== '') {
      filters.push({ column: 'property_type', operator: 'eq', value: formData.property_type });
    }
    ...
    if (formData.bedrooms && formData.bedrooms.trim() !== '') {
      ...
      if (!isNaN(bedroomsNum)) {
        filters.push({ column: 'bedrooms', operator: 'gte', value: bedroomsNum });
      }
    }
    ...
    if (formData.bathrooms && formData.bathrooms.trim() !== '') {
      ...
      if (!isNaN(bathroomsNum)) {
        filters.push({ column: 'bathrooms', operator: 'gte', value: bathroomsNum });
      }
    }
  ...
  getSearchSummary(formData) {
    ...
    if (formData.minPrice || formData.maxPrice) {
      const minPrice = formData.minPrice ? this.formatPrice(parseInt(formData.minPrice)) : '';
      const maxPrice = formData.maxPrice ? this.formatPrice(parseInt(formData.maxPrice)) : '';
    ...
    if (formData.bathrooms) {
      parts.push(`${this.formatBathrooms(parseFloat(formData.bathrooms))}+ bathrooms`);
    }
